...
class MasaCtrlPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwds):
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampler", disable=True)):
    ...
    def invert(
        ...
        **kwds):
        ...
        # print("Valid timesteps: ", reversed(timesteps))
        latents_list = [latents]
        if with_forward and forward_t > 0:
            assert forward_t <= num_inference_steps, \
                f"forward_t ({forward_t}) must be <= num_inference_steps ({num_inference_steps})"

            forward_generator = torch.Generator(device=latents.device).manual_seed(forward_seed)
            ...
            # Perform single forward diffusion: z_0 -> z_forward_t
            timesteps_list = list(timesteps)
            timestep_forward = timesteps_list[forward_t - 1]
            alpha_prod_t = self.scheduler.alphas_cumprod[timestep_forward]
            sqrt_alpha_bar = alpha_prod_t ** 0.5
            sqrt_one_minus_alpha_bar = (1 - alpha_prod_t) ** 0.5
            z_forward = sqrt_alpha_bar * latents + sqrt_one_minus_alpha_bar * fixed_noise
            
            latents_list.append(z_forward)
            latents = z_forward.clone().detach()
            
            # Continue inversion from forward_t onwards
            timesteps = timesteps_list[forward_t:]
            # print(f"Forward diffusion to step {forward_t} (timestep {timestep_forward})")
            # print("Valid timesteps after forward: ", timesteps)

        # pred_x0_list = [latents]
        for i, t in enumerate(tqdm(timesteps, desc="DDIM Inversion", disable=True)):
            if guidance_scale > 1.:
                model_inputs = torch.cat([latents] * 2)
            else:
                model_inputs = latents
